...
int KinectFaces::baseline()
{
	...
	if (false && pSensor2 == nullptr) // was pSensor
	{
		...
		if (FAILED(hResult)) {
			ofLog(OF_LOG_FATAL_ERROR) << "Error : GetDefaultKinectSensor" << std::endl;
			return -1;
		}

		hResult = pSensor2->Open();
		if (FAILED(hResult)) {
			ofLog(OF_LOG_FATAL_ERROR) << "Error : IKinectSensor::Open()" << std::endl;
			return -1;
		}
		...
		if (FAILED(hResult)) {
			ofLog(OF_LOG_FATAL_ERROR) << "Error : IKinectSensor::get_ColorFrameSource()" << std::endl;
			return -1;
		}

		hResult = pSensor2->get_BodyFrameSource(&pBodySource2);
		if (FAILED(hResult)) {
			ofLog(OF_LOG_FATAL_ERROR) << "Error : IKinectSensor::get_BodyFrameSource()" << std::endl;
			return -1;
		}
		...
		if (FAILED(hResult)) {
			ofLog(OF_LOG_FATAL_ERROR) << "Error : IColorFrameSource::OpenReader()" << std::endl;
			return -1;
		}

		hResult = pBodySource2->OpenReader(&pBodyReader2);
		if (FAILED(hResult)) {
			ofLog(OF_LOG_FATAL_ERROR) << "Error : IBodyFrameSource::OpenReader()" << std::endl;
			return -1;
		}
		...
		if (FAILED(hResult)) {
			ofLog(OF_LOG_FATAL_ERROR) << "Error : IColorFrameSource::get_FrameDescription()" << std::endl;
			return -1;
		}
		...
		if (FAILED(hResult)) {
			ofLog(OF_LOG_FATAL_ERROR) << "Error : IKinectSensor::get_CoordinateMapper()" << std::endl;
			return -1;
		}
		...
		for (int count = 0; count < Kinect2552::personCount; count++) {
			...
			if (FAILED(hResult)) {
				ofLog(OF_LOG_ERROR) << "Error : CreateFaceFrameSource" << std::endl;
				return -1;
			}
			...
			if (FAILED(hResult)) {
				ofLog(OF_LOG_ERROR) << "Error : IFaceFrameSource::OpenReader()" << std::endl;
				return -1;
			}
			faces.push_back(face);
		}
